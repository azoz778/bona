// The land price cap and the withheld-listings rule, and the three routes that must all
// enforce them:
...
import { isLandPublic, isPublishable, LAND_PRICE_CAP, WITHHELD_LISTINGS } from '../curate/rules.mjs';
...
test('the land cap never touches houses or apartments', () => {
...
test('withholding is by name, never by price — a house cap would hide a luxury brand\'s best stock', () => {
  // Owner correction 2026-09-08: the SAR 10,000,000 house cap was meant as a one-off removal
  // of two homes, not a standing rule. Price must never withhold a listing again.
  assert.deepEqual([...WITHHELD_LISTINGS].sort(), ['BONA-002', 'BONA-028']);
  assert.equal(isPublishable({ id: 'BONA-002' }), false);
  assert.equal(isPublishable({ id: 'BONA-999', kind: 'house', price: { amount: 500_000_000, currency: 'SAR' } }), true);
  assert.equal(isPublishable(land(500_000_000)), true);
});

test('all three routes enforce the land cap and the withheld list from the one shared rule', () => {
  ...
  assert.match(build, /candidates\.filter\(\(l\) => isPublishable\(l\) && isLandPublic\(l\)\)/, 'and publishes only what passes BOTH rules');
  for (const route of ['scripts/sync-listings.mjs', 'scripts/curate/validate.mjs']) {
    assert.match(src(route), /isLandPublic/, `${route} enforces the land cap`);
    assert.match(src(route), /isPublishable/, `${route} enforces the withheld list`);
  }
});
