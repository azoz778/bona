...
Second pass (2026-09-08, remaining findings), one line per group:

- **A** (Codex) `rollback.sh --copy-back` no longer swallows a failed copy: PC node checked for `node:sqlite` before anything stops, PC API stopped, files staged in `mktemp -d ~/bona-data/.copy-back.XXXXXX` (`bona.db` mandatory, the rest where the VPS has them, "absent on the VPS: f" otherwise), VPS lead count compared with the staged copy, then `mv -f` + `chmod 600`, temp dir removed on every exit path, PC units started last; a PC WAL/SHM the VPS no longer has is removed. Remote count one-liner is `vps_count_leads()` in `lib.sh`, shared with `cutover.sh`.
- **B** (Codex) rollback tests through the shims: plain, `--copy-back` success (files + modes in the temp HOME, temp dir gone), scp fails, `bona.db` missing on the VPS, VPS refuses to stop — exit codes, call order, and never a local `enable --now` on failure. `scp` shim creates the destination for remote→local copies and fails on `SHIM_SCP_FAIL=1` / `SHIM_REMOTE_MISSING`; `ssh` shim answers `test -f ~/bona-data/NAME` from `SHIM_REMOTE_MISSING`.
- **C** (Codex) the cutover happy path asserts the local `is-active --quiet bona-api` / `cloudflared-bona` checks sit between the PC `stop` and the `scp` (the shim already logged them).
- **D** (Claude) preflight-failure scenarios: `SHIM_VPS_ALREADY_ACTIVE=1` (unit live on the VPS) and `SHIM_CHECK_FAIL=1` (`--check` exits 2) → cutover refuses with no local `stop`, no remote `disable --now`, no scp, nothing started.
- **E** (Claude) `deploy.sh` records whether `bona-repo-sync.timer` was active, stops the timer AND `bona-repo-sync.service`, and the EXIT trap (armed before the stop) starts the timer again only if it was active; red tests still mean no restart. `systemctl` shim: `SHIM_LOCAL_INACTIVE`.
- **F** (Claude) `lib.sh` `GIT_BIN=${GIT_BIN:-/usr/bin/git}` renders `@GIT@` in `bona-repo-sync.service.in`; `install-vps.sh --check` gains `git at $GIT_BIN`; tests point `GIT_BIN` at the git shim (ready path), a missing path (`MISSING:` line) and `/snap/bin/git` (render).
- **G** (Codex + Claude) docs: README `BONA_WA_POLL` row (defaults on in-process, opt out in `bona-services.env`, no unit sets it), the PC installer marked legacy / rollback only, runbook `systemctl`/`journalctl` prefixed with `ssh hermes-vps`; spec §3.3 (installer enables nothing, real placeholders incl. `@GIT@`, `bona.yml` from `render_tunnel_config`, `scp` not rsync, tests as they are). Suite: 475 tests.
- H (Codex pass 3): `cutover.sh` removes stale `bona.db-wal`/`bona.db-shm` on the VPS before the copy; the happy-path test asserts it sits between the stop-verification and the scp.
- **I** (first live attempt, 2026-09-08): system units instead of user units — Ubuntu 24.04 sets `kernel.apparmor_restrict_unprivileged_userns=1`, so a `systemctl --user` unit cannot apply the sandbox directives and `bona-api` died with `218/CAPABILITIES` at "start bona-api on the VPS"; the cutover rolled back cleanly. Now: templates carry `User=@USER@`/`Group=@USER@`/`Environment=HOME=@HOME@` and `WantedBy=multi-user.target` (hardening unchanged), `lib.sh` has `VPS_USER`, `BONA_UNIT_DIR` (`/etc/systemd/system`; tests override), `VPS_SYSTEMCTL`/`VPS_JOURNALCTL` (`sudo -n …`); `install-vps.sh` installs with `sudo -n install`, retires the first attempt's user units (its only `systemctl --user`: `disable --now` + reload), `--check` wants the units under `BONA_UNIT_DIR` and passwordless sudo instead of linger; `cutover.sh`/`rollback.sh`/`deploy.sh` use `sudo -n systemctl`/`journalctl` on the VPS (PC side unchanged); the `ssh` shim answers only the `sudo -n` strings, a `sudo` shim logs and execs; `loginctl` shim retired. README §5/§6/§8 and spec §3.2/§3.3/§6 say `sudo systemctl`.
- J (Codex pass 5 + Claude): `install-vps.sh --check` and the cutover preflight refuse while attempt-1 user-unit files or processes remain on the VPS (probes anchored to the binary); the retire step verifies a unit is inactive before deleting its file; root guard; the sudo probe runs `systemctl --version`.
