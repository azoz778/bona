...
- **Matched-only policy** (owner decision): a message is kept only if one of: (1) Ref line `/\bRef\s+(BONA(?:-W?\d{3})?)?\s*[·\-:|]?\s*([A-HJ-NP-Z2-9]{5,6})\b/i`; (2) sender already a lead; (3) `contextInfo.externalAdReply` / `conversionSource` / `entryPointConversionSource` / `utm` present (`match_method:'ad_meta'`, meta stored — checked before the keyword so an ad-origin chat keeps its ad source); (4) mentions `Bona|بونا` or a listing id `BONA-W?\d{3}` (`match_method:'keyword'`, source `whatsapp_organic`); (5) a `whatsapp_click` event within ±15 min whose session has no lead yet and the sender is unknown (`match_method:'time_window'`, closest click, marked inferred). Anything else is **discarded in memory** (counter only).
