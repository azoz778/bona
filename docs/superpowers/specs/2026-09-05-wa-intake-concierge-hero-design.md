...
| **Cloudflare**: `~/.cloudflared/cert.pem` (account-level tunnel cert) + zone-scoped API token. `cloudflared tunnel list` works. Existing tunnel `jarvis-hud-laptop` (not running). | Public HTTPS for a WSL-hosted API = new tunnel `bona` → `api.bona.azoz.uk`. No VPS SSH needed (classifier blocks it). |
...
Owner's phone ──WhatsApp group "Bona …"──▶ Evolution API (wa-api.azoz.uk, owner's own instance)
                     ...
                     api.bona.azoz.uk                 │  /v1/chat/*  → Retell chat (agent "Dana", chat channel)
...
- Custom tools (webhooks, `POST https://api.bona.azoz.uk/v1/tools/<name>?token=<BONA_TOOL_TOKEN>`): `search_properties(kind?, district?, category?, minPrice?, maxPrice?, beds?, query?)`, `show_property(id|slug)`, `create_lead(name?, phone, interest, budget?, timeline?, notes?, language)`. Verify Retell's webhook body shape (`{ call: {...}, args: {...} }` or `{ chat: …, args }`) and respond with a JSON string result.
...
**Tunnel + systemd** (`services/deploy/`): `install.sh` (idempotent) — `cloudflared tunnel create bona` (skip if exists) → `~/.cloudflared/bona.yml` (`ingress: api.bona.azoz.uk → http://localhost:4102`, catch-all 404) → `cloudflared tunnel route dns bona api.bona.azoz.uk` → units `cloudflared-bona.service` and `bona-api.service` (user, `Restart=always`) → `systemctl --user daemon-reload && enable --now` → `curl https://api.bona.azoz.uk/health`. Optional: add an Uptime Kuma HTTP monitor for `/health` using `~/.secrets/uptime-kuma.env` (only if a simple REST/socket path exists; skip otherwise).

**Acceptance.** `node --test services/api/test` green (search, card formatting, rate limit, CORS, tool auth). `curl -X POST https://api.bona.azoz.uk/v1/chat/session -d '{"locale":"ar"}'` returns a greeting; a follow-up message asking for "villa in Al Khalidiyah" returns a `show_listing` action; `/v1/call/token` returns a token; provisioning script is re-runnable. Document all ids + curl examples in `services/README.md`.
